"""
Estou só copiando o código das respostas e entendendo eles:
"""
...
#Metodo do livro (Não funciona aqui):
...
wm.render_to_file('Pagina_526_global_gdp.svg')
'''


"""
O ARQUIVO 'global_gdp.json' QUE EU BAIXEI DA INTERNET NÃO TEM O MESMO CONTEUDO
DO QUE SERIA NECESSÁRIO PARA VER O CODIGO DO LIVRO(PDF) FUNCIONAR. ASSIM NÃO PUDE
SABER DO FUNCIONAMENTO DESTE ALGORITMO. NÃO ENCONTREI O ARQUIVO JSON CORRESPONDENTE
NA NTERNET.
"""
...
class CountryCodesTestCase(unittest.TestCase):
    ...
    def test_Pag_514_Country_Codes(self):
        country_code = pega_cod_pais('United Arab Emirates')
        self.assertEqual(country_code, 'AE') # O codigo Resposta da internet esta com 'ae' Minusculo (Ñ Funfa)

        country_code = pega_cod_pais('Andorra')
        self.assertEqual(country_code, 'AD')

        #Resolvendo do Meu Jeito -> Com UMA Linha:
        self.assertEqual(pega_cod_pais('Afghanistan'), 'AF')
